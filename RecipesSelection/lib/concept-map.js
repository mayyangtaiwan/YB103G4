//in:- //RcpName, //IngreName //RcpTotalCalCat

//out: -  //RcpName, //IngreName IngreUnit IngreQty
//RcpTotalCalCat // //RcpSrc

var ConceptMap = function(chartElementId, infoElementId, dataJson) {

  var a = 1000,//wheel width
    c = 650, //wheel hight
    h = c,
    U = 150, //rcp box width
    K = 22, //rcp box hight
    S = 20, //distance of lines to rcps 
    s = 10, //distance of lines to rcps 
    R = 45, 
    J = 25,
    o = 15,//rcps text postion
    t = 20, //rcps text padding
    w = 1000,
    F = "elastic",
    N = "#ffa100"; //org #0da4d3- 點到線的顏色
  ...
  function G(Y, X) {
    if (L.node === Y && X !== true) {
      if (Y.type === "ditem") {
        window.location.href = "/select/" + Y.slug;
        return
      }
  ...
  function C(Z) {
    ...
    if (ab && ab.type === "ditem") {
      var aa = Y.append("a").attr("xlink:href", function(ae) {
        return "/select/" + ae.slug
      });
      aa.append("text").attr("fill", N).attr("text-anchor", "middle").attr("y", (o + t) * -1).text(function(ae) {
        return "推薦食譜" + ae.ditem
      })
    } else {
      if (ab && ab.type === "theme") {
        Y.append("text").attr("fill", "green").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("食材")
      } else {
        if (ab && ab.type === "perspective") {
          ...
          Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("食譜熱量");
          ad.exit().remove()
        }
      }
    }
    ...
    X.enter().append("text").attr("text-anchor", "start").attr("x", a / -2 + t).attr("y", c / 2 - t).text("重新選擇食譜！").attr("class", "all-ditems").on("click", O);
    X.exit().remove()
  }
  ...
  function m(Y) {
    ...
    }).attr("stroke", "#000").attr("stroke-width", 8.5).transition().duration(w).ease(F).attr("d", function(Z) {
      return v([
        [Z.x1, Z.y1],
        [Z.xx, Z.yy],
        [Z.x2, Z.y2]
  ...
  function z() {
    ...
    E.selectAll("text.label").attr("fill", function(X) {
      return (X === L.node || X.isGroup) ? "#fff" : l(X, "#000", N, "#000")
    })
  }
